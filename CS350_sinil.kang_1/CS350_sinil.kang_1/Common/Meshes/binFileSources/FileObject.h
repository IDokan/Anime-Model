...
public:
	...
	~FileObject(void);
	template<class T>
	void Read(T& obj) { fread(&obj, sizeof(obj), 1, pFile_); }
	void Read(std::string& obj);
	template<class T>
	void Read(T& obj, unsigned int Count) { fread(&obj, sizeof(obj), Count, pFile_); }
	template<class T>
	void Write(T& obj) { fwrite(&obj, sizeof(obj), 1, pFile_); }
	void Write(std::string& obj);
	template<class T>
	void Write(T& obj, unsigned int Count) { fwrite(&obj, sizeof(obj), Count, pFile_); }
